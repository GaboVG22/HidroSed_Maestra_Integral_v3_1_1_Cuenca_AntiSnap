...
st.markdown(
    """
<div class='hs-hero'>
  <h1>🌊 HidroSed Maestra Integral v3.1.1 Cuenca + Curvas · Dios Supremo · Hotfix Cuenca Anti-Snap · Topo Opcional</h1>
...
with tabs[2]:
    ...
    if not has("dem_path") or not has("control_point"):
        ...
        with c1:
            ...
            snap_radius = st.selectbox("Radio ajuste punto al cauce [m]", [50, 100, 250, 500, 1000, 1500, 2500, 5000], index=2)
        with c2:
            expected_area = st.number_input("Área esperada aprox. [km²]", min_value=0.0, value=20.0, step=5.0)
            basin_area_limit = st.number_input("Área máxima permitida [km²]", min_value=1.0, value=100.0, step=10.0)
        with c3:
        ...
        st.info("Si esperas una cuenca de 20 km², no uses el modo antiguo de máxima acumulación ni radios grandes: puede ajustar el punto al río principal y devolver miles de km².")
